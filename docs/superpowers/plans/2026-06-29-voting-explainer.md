# Voting Methods Explainer Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build an interactive `/learn` page that step-by-step explains Plurality, Borda (Ranked Choice), and Ranked Pairs voting using a fixed curated scenario, and link to it from the home page.

**Architecture:** A single new `LearnPage.tsx` page component containing all step content as static JSX — no data fetching, no computation. A reusable `MethodCard` local component owns per-card step state via `useState`. Two existing files are edited: `App.tsx` gets the new route, `CreatePoll.tsx` gets a "How does this work?" link.

**Tech Stack:** React 18, React Router 6, Tailwind CSS 3 with project oklch design tokens, TypeScript

## Global Constraints

- Use existing CSS classes only: `.card`, `.btn-secondary`, `.badge`, `.input`, `.btn-primary`; custom values must use project oklch tokens from `index.css`
- Font: Bricolage Grotesque (inherited from body — no explicit font-family needed)
- Dark glassmorphism aesthetic: all new surfaces use `bg-[var(--raised-glass)] backdrop-blur-md border border-line rounded-2xl` (i.e. `.card`)
- No new npm dependencies
- `npm run build` (from `frontend/`) must exit 0 with no TypeScript errors after each task
- No backend changes — pure frontend

---

### Task 1: Create `LearnPage.tsx` with full step-through content
...
- Produces: `export function LearnPage()` — used by Task 2

- [ ] **Step 1: Create `frontend/src/pages/LearnPage.tsx` with the full implementation**

Write the entire file as shown below. This is the complete, final file — no placeholders.
...
Expected: exits 0, no TypeScript errors. Vite outputs JS and CSS bundles.

- [ ] **Step 3: Commit**

```bash
git add frontend/src/pages/LearnPage.tsx
git commit -m "feat: add voting methods explainer page (LearnPage)"
```

---

### Task 2: Wire up the `/learn` route and link from CreatePoll
...
**Interfaces:**
- Consumes: `LearnPage` exported from `frontend/src/pages/LearnPage.tsx` (Task 1)
- Produces: nothing consumed by later tasks

**Context on `App.tsx`:**

The current routes in `App.tsx` are:
...
Add the `/learn` route before the catch-all `*` route.

**Context on `CreatePoll.tsx`:**

The voting method selector is around line 14–18 (the `VOTING_METHODS` array) and renders inside the form. Find the section where `VOTING_METHODS.map(...)` renders the method buttons. Add a small link directly below that section.

The exact render location: search for the JSX that maps over `VOTING_METHODS`. It renders method selector buttons. Immediately after the closing `</div>` of that map, add the link.

- [ ] **Step 1: Add the `/learn` route to `App.tsx`**

Read `frontend/src/App.tsx` first. Then make these two changes:

1. Add the import at the top with the other page imports:
```tsx
import { LearnPage } from './pages/LearnPage'
```

2. Add the route before the catch-all:
```tsx
...
- [ ] **Step 2: Add the "How does this work?" link to `CreatePoll.tsx`**

Read `frontend/src/pages/CreatePoll.tsx` to find the exact location.

The voting method selector renders something like:
...
`Link` is already imported from `react-router-dom` in `CreatePoll.tsx` — do not add a duplicate import. Verify by checking the top of the file.
...
```bash
git add frontend/src/App.tsx frontend/src/pages/CreatePoll.tsx
git commit -m "feat: add /learn route and link from CreatePoll voting method selector"
```
